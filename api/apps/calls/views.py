...
class BillViewSet(viewsets.ModelViewSet):
    def create(self, request, *args, **kwargs):
        ...
        if not source:
            return Response("Invalid request. Missing source parameter.",status=status.HTTP_400_BAD_REQUEST)
            
        if period:
            queryset = CallRecord.objects.filter(source=source).filter(end_date__year=period.split('/')[1], end_date__month=period.split('/')[0])
        else:
            default_date = date.today().replace(day=1) - timedelta(days=1)
            bill['period'] = f'{default_date.month}/{default_date.year}'
            queryset = CallRecord.objects.filter(source=source).filter(end_date__year=default_date.year, end_date__month=default_date.month)
    ...
    def get_call_record_price(self, call_start, call_end, duration):
        ...
        if call_end.time() >= call_start.time():
            if time_limit_start <= call_start.time() <= time_limit_end:
                ...
                if time_limit_start <= call_end.time() <= time_limit_end:
                    ...
                    charge = datetime.combine(date.min, time_limit_end) - datetime.combine(date.min, time_limit_start)
        else:
            if time_limit_start <= call_start.time() <= time_limit_end:
                charge = datetime.combine(date.min, time_limit_end) - datetime.combine(date.min, call_start.time())
            if time_limit_start <= call_end.time() <= time_limit_end:
        ...
        if duration.days > 0:
            charge_per_day = datetime.combine(date.min, time_limit_end) - datetime.combine(date.min, time_limit_start)
            total_minute_charge += int(charge_per_day.total_seconds()/60) * duration.days
